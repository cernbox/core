...
//							}
	...
	addShareWith:function(shareType, shareWith, shareWithDisplayName, permissions, possiblePermissions, mailSend, collection) {
		...
		if (collection) {
			...
			// HUGO HIDE CHECKBOX TO DO OUR CUSTOM READ WRITE DELETE
			
			html += '<label><input type="checkbox" name="read" class="permissions" checked="true" disabled=true />'+'read'+'</label> ';
			if (possiblePermissions & OC.PERMISSION_CREATE) {
				html += '<label><input type="checkbox" name="create" class="permissions" '+createChecked+' data-permissions="'+OC.PERMISSION_CREATE+'" />'+'write+delete'+'</label>';
			}
			/*
			if (possiblePermissions & OC.PERMISSION_UPDATE) {
