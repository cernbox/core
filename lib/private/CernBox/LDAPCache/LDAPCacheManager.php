...
class LDAPCacheManager
{
	...
	public static function addUserEGroup($user, $group, $membership)
	{
		$key = self::REDIS_KEY_GROUP_MAPPING_CACHE . $user;
		Redis::writeToCacheMap($key, $group, $membership);
	}
	 ...
	 * @param unknown $group
	 * 
	 * @return membership (string either "1" or "") or FALSE if the group is not in the cache
	 */
	public static function getUserMembershipToGroup($user, $group)
	{
		$redisKey = self::REDIS_KEY_GROUP_MAPPING_CACHE . $user;
		return Redis::readFromCacheMap($redisKey, $group);
	}
