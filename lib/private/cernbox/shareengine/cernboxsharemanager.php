...
final class CernboxShareManager implements IManager
{
	 ...
	 */
	public function __construct(ILogger $logger, IConfig $config, ISecureRandom $secureRandom, IHasher $hasher, IMountManager $mountManager, IGroupManager $groupManager, IL10N $l, IProviderFactory $factory, IUserManager $userManager, IRootFolder $rootFolder)
	{
		$this->logger = $logger;
		$this->config = $config;
	...
	protected function generalCreateChecks(\OCP\Share\IShare $share)
	{
		...
		{
			$message_t = $this->l->t('You are not allowed to share %s', [ 
					$share->getNode()->getPath() 
			]);
		...
		{
			$message_t = $this->l->t('Cannot increase permissions of %s', [ 
					$share->getNode()->getPath() 
			]);
			throw new GenericShareException($message_t, $message_t, 404);
		}
	 ...
	 *
	 * @param \OCP\Share\IShare $share
	 *        	The share to validate the expiration date of
	...
	protected function linkCreateChecks(\OCP\Share\IShare $share)
	{
		...
		if (! $this->shareApiLinkAllowPublicUpload() && ($share->getPermissions() & (\OCP\Constants::PERMISSION_CREATE | \OCP\Constants::PERMISSION_UPDATE)))
		{
	 ...
	 * of a link if it is a reshare.
	 * This is a quick word around
	 ...
	 * @throws \Exception TODO: handle link share permissions or check them
	 */
	...
	{
		if ($path !== null && ! ($path instanceof \OCP\Files\File) && ! ($path instanceof \OCP\Files\Folder))
		{
	...
	public function getSharesByPath(\OCP\Files\Node $path, $page = 0, $perPage = 50)
	{
		...
		try
		{
			$temp = $provider->getSharesByPath($path);
			$tempCount = count($temp);
			
			// If we dont have enough to fill 1 page, we always return all the shares (no matter which page)
			if($tempCount < $perPage)
			{
				$shares = array_merge($shares, $temp);
			}
			// If we have enough or more for 1 page, we get the shares for that page
			// If client request a page we cannot fill because insuficent shares, we return empty array
			else if($tempCount >= $start)
			{
				if($tempCount < $end)
				{
					$end = $tempCount;	
		...
		{
			\OCP\Util::writeLog('SHARE ENGINE', 'Manager: Failed at getSharesByPath(): ' . $e->getMessage(), \OCP\Util::ERROR);
		}
	...
	{
		return $this->shareApiLinkDefaultExpireDate() && $this->config->getAppValue('core', 'shareapi_enforce_expire_date', 'no') === 'yes';
	}
