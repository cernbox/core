 ...
 */
final class CernboxSession extends \OC\User\Session
{
	...
	public function setUpNewUser($uid)
	{
		...
		$uidAndGid = EosUtil::getUidAndGid($uid);
		if($uidAndGid === false) 
		{
		...
		$homedir = EosUtil::getEosPrefix() . substr($uid, 0, 1) . "/" . $uid . "/";
		//$meta = \OC\Files\ObjectStore\EosUtil::getFileByEosPath($homedir);
		
		$errorCode = -1;
		do
		{
			$errorCode = EosUtil::statFile($homedir);
			if($errorCode != EosUtil::STAT_FILE_EXIST)
			{
				usleep(500000); // Half a second delay between request (microseconds)
			}
		}
		while($errorCode != EosUtil::STAT_FILE_EXIST && $errorCode != EosUtil::STAT_FILE_NOT_EXIST);
		
		// path exists so we let the user access the system
		if($errorCode === EosUtil::STAT_FILE_EXIST) 
		{
			\OCP\Util::writeLog('EOSLOGIN', "user: $uid has a valid homedir that is: $homedir", \OCP\Util::ERROR);
			
			EosUtil::getFolderContents($homedir, function(array &$data) use ($uid)
			{
