...
class EosParser {
	...
	public static function parseRecycleLsMonitorMode($line_to_parse) {
		...
		*/
		// filter files to only show deleted files under the /user/l/labrador/cernbox folder and not from the whole tree
		//$path = $info['restore-path'];
		/*$filter = $eos_prefix . substr($user, 0, 1) . "/" . $user . "/cernbox";
	if (strpos($path, $filter) === 0) {
	$files[] = $info;
	}*/
	}

	// parses the extended attribute user.acl and return a map or false if the alc is empty or has a sintax error
	/*
	public static function parseAcl($alc){
		$usersWithPermissions = split($acl, ",");
		if($usersWithPermissions){
			$primaryUser = "";
			$splitted = split($usersWithPermissions, ":");
			if(count($splitted) >= 3){
				$type = $splitted[0];
				$username = $splitted[1];
	...
	// parse ls -la
	public static function parseLsMinusLA($line){
		$ocPerm = 0;
		if($line) {
