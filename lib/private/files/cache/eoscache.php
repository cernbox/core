...
class EosCache {
	...
	*/
	...
	public function get($ocPath) {
		...
		// I had it TRU before. Maybe some side effects. I have to test all the scenarios
		if(!$eosPath){ 
			return false;
		}
		list($uid, $gid) = EosUtil::getEosRole($eosPath, true);
		$get     = "eos -b -r $uid $gid  file info \"" . $eosPath . "\" -m";
		$result  = null;
		$errcode = null;
		$info    = array();
		exec($get, $result, $errcode);
		if ($errcode !== 0) { // if we try to obtain the info for first time of user root and it does not exists we have to create it
			// Very verbose and wee see lot of entries in the log
			//\OCP\Util::writeLog('eos', "get $get $errcode",\OCP\Util::ERROR);
			//check if the path is a user root 
			if(strpos($eosPath, $eos_prefix) === 0){
				$rest = substr($eosPath, strlen($eos_prefix));
				$parts = explode("/", $rest);
				if(count($parts) === 3 && $parts[2] === "" && $parts[0] === substr($parts[1], 0,1)){ // we are sure we only obtain l/labrador/ and nothing more
					// create the user files root
					$cmd = "eos -b -r 0 0 mkdir \"$eosPath\"";
					$result          = null;
					$errcode         = null;
					...
					$result = null;
					$errcode = null;
					$cmd2 = "eos -b -r 0 0 chown -r $uid:$gid $eosPath";
					exec($cmd2, $result, $errcode);
					if($errcode !== 0){
						\OCP\Util::writeLog('eos', "eoschown $cmd2 $errcode", \OCP\Util::ERROR);
						return false;
					}
					// add the user created to sys.acl
					/*
					$username = $parts[1];
					$result = null;
					$errcode = null;
					$cmd3 = "eos -b -r 0 0 attr -r set sys.acl=u:$username:rwx+d \"$eosPath\"";
					exec($cmd3, $result, $errcode);
					if($errcode !== 0){
						\OCP\Util::writeLog('eos', "eosattrset $cmd3 $errcode", \OCP\Util::ERROR);
						return false;
					}
					*/
					return $this->get($ocPath);
				} else {
					return false;
				}
			} else {
	...
	public function getFolderContents($ocPath) {
		...
		//list($uid, $gid) = EosUtil::getEosRole($eosPath, true);
		$uid = 0; $gid = 0;
		$getFolderContents = "eos -b -r $uid $gid  find --fileinfo --maxdepth 1 \"" . $eosPath . "\"";
		$result            = null;
		$errcode           = null;
		$files             = array();
