...
DISK_FILLED=$(df |  grep data | awk '{print $5}')
DISK_VOLUME_MAX_HUMAN=$(df -h |  grep data | awk '{print $2}')
DISK_VOLUME_USED_HUMAN=$(df -h |  grep data | awk '{print $3}')
DISK_VOLUME_MAX=$(df |  grep data | awk '{print $2}')
DISK_VOLUME_USED=$(df |  grep data | awk '{print $3}')
TOTAL_SIZE_BYTES=$(find /data/thumbnails  -type f -printf "%T+\t%s\t%p\n" | sort -n -k 2 | awk '{TOTAL_SIZE+=$2} END {print TOTAL_SIZE'})
NUMBER_OF_FILES=$(find /data/thumbnails  -type f -printf "%T+\t%s\t%p\n" | wc -l)
...
find /data/thumbnails  -type f -printf "%T+\t%s\t%p\n" | sort -n -k 1 | head -${FILES_TO_DELETE}
